...
function tagTemplate(recipe){
    return recipe.tags.map((tag)=> `<p>${tag}</p>`).join(' ');
}
...
function search() {
    ...
    let filteredSnacks = recipes.filter(function(snack){
        return ( 
            snack.name.toLowerCase().includes(query.toLowerCase()) ||
            snack.description.toLowerCase().includes(query.toLowerCase()) || 
            snack.tags.find(tag => tag.toLowerCase().includes(query.toLowerCase()))
        );
    })
    // clear out any previous content
    mainContainer.innerHTML = '';
    // output onto screen
    filteredSnacks.forEach(function(snack){
      renderSnack(snack);
    })
}
...
function init() {
    renderSnack(recipes[randomNum]);
}

function renderSnack(recipe) {
    console.log(recipe);
